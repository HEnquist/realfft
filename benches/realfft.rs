...
fn bench_realfft(b: &mut Bencher, len: usize) {
    ...
    let mut fft = RealToComplex::<f64>::new(len);

    let signal = vec![0_f64; len];
    let mut spectrum = vec![Complex{re: 0_f64, im: 0_f64}; len/2+1];
...
fn bench_pow2(c: &mut Criterion) {
    ...
    for i in [64, 256, 65536].iter() {
        group.bench_with_input(BenchmarkId::new("Complex", i), i, 
            |b, i| bench_fft(b, *i));
        group.bench_with_input(BenchmarkId::new("Real", i), i, 
            |b, i| bench_realfft(b, *i));
    }
    group.finish();
}
